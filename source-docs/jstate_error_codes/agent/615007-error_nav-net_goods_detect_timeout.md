...
<!-- Source: nav-navigation-errors/615007-error_nav-net_goods_detect_timeout.md -->

# JState 错误码 615007：载货失败，货架偏移检测超时

> Agent 视图：仅保留结论、诊断决策、证据/输出契约和 Playbook。完整原文见 [615007-error_nav-net_goods_detect_timeout.md](../nav-navigation-errors/615007-error_nav-net_goods_detect_timeout.md)。
...
```bash
grep -E 'goodsDeviation detect timeout' /opt/jz/log/nav-net.log | tail -n 200
```

| 顺序 | 检查项 | 正常判据 | 异常判据 | 结论状态与下一步 |
|---:|---|---|---|---|
| 1 | 原始错误上报 | 同一时间窗命中 `615007` 或直接触发日志 | 无匹配 | 扩大时间窗并核对进程、版本；标记 `证据不足` |
...
```yaml
error_code: "615007"
error_name: "goods_detect_timeout"
analysis_status: 待确认
applicable_version:
...
playbook:
  ...
  - id: classify_trigger
    ...
    - type: grep_log
      module: nav-net
      pattern: goodsDeviation detect timeout
      on_match:
  ...
  - id: insufficient_data
    ...
    - type: grep_log
      module: nav-net
      pattern: 615007|goods_detect_timeout
      on_match:
  ...
  - id: verify
    ...
    checks:
    - type: verify_fix
      grep_pattern: goodsDeviation detect timeout
