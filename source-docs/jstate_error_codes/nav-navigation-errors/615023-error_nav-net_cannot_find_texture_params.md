# JState 错误码 615023：未检测到纹理配置文件：texture_mapping.json

> 文档用途：当 AI Agent 在现场日志中发现 `615023` 时，用本文理解错误的注册信息和功能背景。
...
|---|---|
| 错误码 | `615023` |
| JState Key | `/jstate/error/nav-net/cannot_find_texture_params` |
| 错误描述 | 未检测到纹理配置文件：texture_mapping.json |
| 等级 | `level1` |
...
**核心判断**：`615023` 在 `health_info.hpp` 的 `HealthType` 和 `kHealthTypeMap` 中注册，但源码中无 `pubJ3State` 调用。可能通过旧版 `set()`+`publish()` 路径发布到 `/agent/health/in`，或通过 Drogon HTTP API 由外部服务触发。
...
```bash
grep -E "cannot_find_texture_params" /opt/jz/log/nav-net.log | tail -n 300
```
...
```yaml
error_code: 615023
error_name: cannot_find_texture_params
analysis_status: 待确认
...
playbook:
  ...
  steps:
    - id: classify_trigger_chain
      description: "确认 cannot_find_texture_params 是否出现"
      checks:
        - type: grep_log
          module: "nav-net"
          pattern: "cannot_find_texture_params"
          on_match:
            trigger_chain: "cannot_find_texture_params_confirmed"
            conclusion: "命中 cannot_find_texture_params"
            goto: diagnose_cannot_find_texture_params
        - type: on_no_match
          conclusion: "未找到日志，可能外部触发或旧版路径"
          action: expand_time_window
          time_window: 120

    - id: diagnose_cannot_find_texture_params
    ...
    - id: verify
      ...
      checks:
        - type: verify_fix
          grep_pattern: "cannot_find_texture_params"
