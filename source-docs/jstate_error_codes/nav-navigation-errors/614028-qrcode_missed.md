...
playbook:
  ...
  steps:
    # ── 步骤 1：区分触发链（起步/切换 vs 导航结束）──
    - id: classify_trigger_chain
      description: "根据日志区分二维码缺失发生在起步/切换阶段还是导航结束阶段"
      checks:
        - type: grep_log
          module: "nav-manager"
          pattern: "二维码导航起步时二维码缺失"
          on_match:
            trigger_chain: "start_or_switch"
            conclusion: "命中 起步/切换阶段二维码缺失，在进入HETEMAP定位时无有效二维码"
            goto: diagnose_start_or_switch
        - type: grep_log
          module: "nav-manager"
          pattern: "二维码导航停止时二维码缺失"
          on_match:
            trigger_chain: "finish"
            conclusion: "命中 导航结束阶段二维码缺失，到站时未识别到目标码"
            goto: diagnose_finish
        - type: on_no_match
    ...
    # ── 步骤 2a：起步/切换阶段深入排查 ──
    - id: diagnose_start_or_switch
      description: "起步/切换阶段排查：定位类型 → topic 发布 → landmark ID → 消息时效"
      checks:
        # 检查定位类型
        - type: grep_log
          module: "nav-manager"
          pattern: "qrcodeCheck loc_type not hetemap"
          priority: P1
          on_match:
            conclusion: "定位类型非 HETEMAP，本应跳过二维码检查但进入了检查链，版本或状态异常"
            root_cause: "定位类型与二维码检查路径不一致"
            temp_fix: "核对 /nav/localization_status 的 value 字段和 name 字段"
            goto: verify
          on_no_match:
            continue: true
        # 检查 topic 发布
        - type: ros_topic_check
          topic: "/sensor_detector/visualmarks"
          online_only: true
          priority: P1
          checks:
            - type: extract_field
              source_step: classify_trigger_chain
              patterns:
                has_publisher: "publishers:"
          on_match:
            conclusion: "visualmarks topic 有发布者"
            continue: true
          on_no_match:
            conclusion: "/sensor_detector/visualmarks 无发布者或不可达"
            root_cause: "二维码消息发布链异常（相机、sensor_detector 或 ROS 通信）"
            temp_fix: "安全停止受影响任务，恢复消息链后重试"
            goto: verify
        # 检查 landmark ID
        - type: grep_log
          module: "nav-manager"
          pattern: "visualmarks id is not detected"
          priority: P1
          on_match:
            conclusion: "二维码未识别，landmark ID <= -1"
            root_cause: "二维码识别失败（码质量、光照、相机参数或安装位置）"
            temp_fix: "清洁并重新对准目标码，确认曝光和视野后重试"
            goto: verify
          on_no_match:
            continue: true
        # 检查消息时效
        - type: grep_log
          module: "nav-manager"
          pattern: "visualmarks message exceeds age threshold"
          priority: P1
          on_match:
            conclusion: "二维码消息过期（年龄 >= 1s），ID 有效但时间戳过旧"
            root_cause: "sensor_detector 发布频率不足、处理阻塞或 ROS 时间异常"
            temp_fix: "恢复发布频率和时间同步后再执行任务"
            goto: verify
          on_no_match:
            continue: true
        # 检查起步时序
        - type: grep_log
          ...
          context_lines: 5
          on_match:
            conclusion: "任务开始日志存在，刚进入HETEMAP即检查，可能无新鲜数据"
            root_cause: "起步/切换时序过早，定位刚切换到 HETEMAP 尚无新鲜 landmark"
            temp_fix: "等待二维码定位真正就绪后重发任务"
            goto: verify
          on_no_match:
            continue: true
        # 兜底
        - type: on_no_match
          conclusion: "起步/切换阶段的所有已知特征均未命中，当前证据不足"
          status: insufficient_data
          action: "收集 /sensor_detector/visualmarks 原始消息和更长时间窗日志"

    # ── 步骤 2b：导航结束阶段深入排查 ──
    - id: diagnose_finish
      description: "导航结束阶段排查：topic 发布 → landmark ID → 消息时效 → 目标码可见性"
      checks:
        # 检查 topic 发布
        - type: grep_log
          module: "nav-manager"
          ...
          priority: P1
          on_match:
            conclusion: "visualmarks 日志存在，继续检查内容"
            continue: true
          on_no_match:
            conclusion: "/sensor_detector/visualmarks 无消息或无日志"
            root_cause: "到站时二维码消息链中断"
            temp_fix: "安全停止受影响任务，恢复消息链后重试"
            goto: verify
        # 检查 landmark ID
        - type: grep_log
          ...
          priority: P1
          on_match:
            conclusion: "到站时二维码未识别，目标码可能离开视野或被遮挡"
            root_cause: "停车位导致目标码离开视野、被遮挡或未安装"
            temp_fix: "调整车辆到可识别位置后安全重试"
            goto: verify
          on_no_match:
            continue: true
        # 检查消息时效
        - type: grep_log
          ...
          priority: P1
          on_match:
            conclusion: "到站时二维码消息过期（年龄 >= 1s）"
            root_cause: "sensor_detector 发布频率不足或 ROS 时间异常"
            temp_fix: "恢复发布频率和时间同步"
            goto: verify
          on_no_match:
            continue: true
        # 检查 movebase finished
        - type: grep_log
          module: "nav-manager"
          pattern: "movebase is finished"
          priority: P2
          on_match:
            conclusion: "move_base 完成，进入结束检查"
            continue: true
          on_no_match:
            continue: true
        # 兜底
        - type: on_no_match
          conclusion: "导航结束阶段的所有已知特征均未命中，当前证据不足"
          status: insufficient_data
          action: "收集到站前后的 visualmarks 原始消息、定位状态和更长时间窗日志"

    # ── 步骤 3：验证修复 ──
    - id: verify
      description: "验证修复：连续 60 秒不再出现二维码缺失，且 visualmarks 数据正常"
      checks:
        - type: verify_fix
          grep_pattern: "二维码导航.*时二维码缺失"
          watch_duration_sec: 60
          on_match:
            status: still_present
            action: "二维码缺失仍然存在，扩大排查范围或升级人工处理"
          on_no_match:
            status: verified
            conclusion: "连续 60 秒未出现二维码缺失，错误已清除"
```
