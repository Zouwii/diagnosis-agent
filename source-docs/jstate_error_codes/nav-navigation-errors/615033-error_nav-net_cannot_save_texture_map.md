# JState 错误码 615033：保存纹理地图文件失败

> 文档用途：当 AI Agent 在现场日志中发现 `615033` 时，用本文理解错误的注册信息和功能背景。
...
|---|---|
| 错误码 | `615033` |
| JState Key | `/jstate/error/nav-net/cannot_save_texture_map` |
| 错误描述 | 保存纹理地图文件失败 |
| 等级 | `level5` |
...
**核心判断**：`615033` 在 `health_info.hpp` 的 `HealthType` 和 `kHealthTypeMap` 中注册，但源码中无 `pubJ3State` 调用。可能通过旧版 `set()`+`publish()` 路径发布到 `/agent/health/in`，或通过 Drogon HTTP API 由外部服务触发。
...
```bash
grep -E "cannot_save_texture_map" /opt/jz/log/nav-net.log | tail -n 300
```
...
```yaml
error_code: 615033
error_name: cannot_save_texture_map
analysis_status: 待确认
...
playbook:
  ...
  steps:
    - id: classify_trigger_chain
      description: "确认 cannot_save_texture_map 是否出现"
      checks:
        - type: grep_log
          module: "nav-net"
          pattern: "cannot_save_texture_map"
          on_match:
            trigger_chain: "cannot_save_texture_map_confirmed"
            conclusion: "命中 cannot_save_texture_map"
            goto: diagnose_cannot_save_texture_map
        - type: on_no_match
          conclusion: "未找到日志，可能外部触发或旧版路径"
          action: expand_time_window
          time_window: 120

    - id: diagnose_cannot_save_texture_map
    ...
    - id: verify
      ...
      checks:
        - type: verify_fix
          grep_pattern: "cannot_save_texture_map"
