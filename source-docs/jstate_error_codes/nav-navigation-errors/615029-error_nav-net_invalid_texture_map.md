# JState 错误码 615029：无效的纹理地图名称

> 文档用途：当 AI Agent 在现场日志中发现 `615029` 时，用本文理解错误的注册信息和功能背景。
...
|---|---|
| 错误码 | `615029` |
| JState Key | `/jstate/error/nav-net/invalid_texture_map` |
| 错误描述 | 无效的纹理地图名称 |
| 等级 | `level5` |
...
**核心判断**：`615029` 在 `health_info.hpp` 的 `HealthType` 和 `kHealthTypeMap` 中注册，但源码中无 `pubJ3State` 调用。可能通过旧版 `set()`+`publish()` 路径发布到 `/agent/health/in`，或通过 Drogon HTTP API 由外部服务触发。
...
```bash
grep -E "invalid_texture_map" /opt/jz/log/nav-net.log | tail -n 300
```
...
```yaml
error_code: 615029
error_name: invalid_texture_map
analysis_status: 待确认
...
playbook:
  ...
  steps:
    - id: classify_trigger_chain
      description: "确认 invalid_texture_map 是否出现"
      checks:
        - type: grep_log
          module: "nav-net"
          pattern: "invalid_texture_map"
          on_match:
            trigger_chain: "invalid_texture_map_confirmed"
            conclusion: "命中 invalid_texture_map"
            goto: diagnose_invalid_texture_map
        - type: on_no_match
          conclusion: "未找到日志，可能外部触发或旧版路径"
          action: expand_time_window
          time_window: 120

    - id: diagnose_invalid_texture_map
    ...
    - id: verify
      ...
      checks:
        - type: verify_fix
          grep_pattern: "invalid_texture_map"
