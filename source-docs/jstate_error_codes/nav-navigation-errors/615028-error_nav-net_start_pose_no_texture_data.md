# JState 错误码 615028：起点无纹理信息

> 文档用途：当 AI Agent 在现场日志中发现 `615028` 时，用本文理解错误的注册信息和功能背景。
...
|---|---|
| 错误码 | `615028` |
| JState Key | `/jstate/error/nav-net/start_pose_no_texture_data` |
| 错误描述 | 起点无纹理信息 |
| 等级 | `level5` |
...
**核心判断**：`615028` 在 `health_info.hpp` 的 `HealthType` 和 `kHealthTypeMap` 中注册，但源码中无 `pubJ3State` 调用。可能通过旧版 `set()`+`publish()` 路径发布到 `/agent/health/in`，或通过 Drogon HTTP API 由外部服务触发。
...
```bash
grep -E "start_pose_no_texture_data" /opt/jz/log/nav-net.log | tail -n 300
```
...
```yaml
error_code: 615028
error_name: start_pose_no_texture_data
analysis_status: 待确认
...
playbook:
  ...
  steps:
    - id: classify_trigger_chain
      description: "确认 start_pose_no_texture_data 是否出现"
      checks:
        - type: grep_log
          module: "nav-net"
          pattern: "start_pose_no_texture_data"
          on_match:
            trigger_chain: "start_pose_no_texture_data_confirmed"
            conclusion: "命中 start_pose_no_texture_data"
            goto: diagnose_start_pose_no_texture_data
        - type: on_no_match
          conclusion: "未找到日志，可能外部触发或旧版路径"
          action: expand_time_window
          time_window: 120

    - id: diagnose_start_pose_no_texture_data
    ...
    - id: verify
      ...
      checks:
        - type: verify_fix
          grep_pattern: "start_pose_no_texture_data"
