...
- 新增 `docs/archive/` 目录，存放已过时的历史设计方案。
...
- `docs/archive/00-项目初始化与Git策略.md`：历史多 Agent/12 节点设计下的 Git 策略、目录权限和部署架构，与当前实现不一致。
- `docs/archive/02-功能模块设计.md`：历史 12 节点 StateGraph、多 Agent 协作设计，当前图只有单节点。
- `docs/archive/05-开发指南.md`：历史 10 Phase 开发计划，面向子进程 CLI、未迁移 engine、未实施 knowledge 的阶段，已过时。
